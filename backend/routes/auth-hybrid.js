...
router.post('/register', [
    ...
    body('password').isLength({ min: 8 }).withMessage('Password must be at least 8 characters')
        ...
        .not().matches(/^(.)\1{2,}/).withMessage('Password cannot have repeated characters (e.g., aaa, 111)')
        .custom((value) => {
            ...
            try {
                ...
                if (user) {
                    ...
                    if (validPassword) {
                        ...
                        const lockoutDuration = parseInt(process.env.LOCKOUT_DURATION) || 900; // seconds (15 min)
                        ...
                        if (failedAttempts >= maxAttempts) {
                            ...
                            await db.query(
                                'UPDATE users SET failed_login_attempts = ?, locked_until = ?, last_failed_login = NOW() WHERE id = ?',
                            ...
                            return res.status(423).json({
                                success: false,
                                error: {
                                    message: `Account locked due to ${maxAttempts} failed login attempts. Please try again in ${Math.ceil(lockoutDuration / 60)} minutes.`,
                                    status: 423,
