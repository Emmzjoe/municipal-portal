...
function loadUsersTable() {
    ...
    allUsers.forEach(user => {
        ...
        let outstanding = 0;
        allBills.forEach(bill => {
            if (bill.account_number === user.account_number &&
        ...
        row.innerHTML = `
            ...
            <td>${user.property_type || 'Residential'}</td>
            <td>N$ ${outstanding.toFixed(2)}</td>
            <td>
                <button class="btn btn-secondary" onclick="viewUserDetails('${user.account_number}')">View</button>
            </td>
        `;
...
function loadBillsTable() {
    ...
    allBills.forEach(bill => {
        ...
        const user = allUsers.find(u => u.account_number === bill.account_number);
        ...
        row.innerHTML = `
            ...
            <td>
                ${statusLower !== 'paid' ? `<button class="btn btn-primary" onclick="markAsPaid(${bill.bill_id})">Mark Paid</button>` : '<span style="color: var(--success);">✓ Paid</span>'}
            </td>
        `;
        tbody.appendChild(row);
    });
...
function calculateMonthlyRevenue() {
    ...
    months.forEach((month, index) => {
        ...
        allPayments.forEach(payment => {
            if (payment.status && payment.status.toLowerCase() === 'success' && payment.created_at) {
...
function loadTransactions() {
    ...
    recentPayments.forEach(payment => {
        const date = payment.created_at ? new Date(payment.created_at).toLocaleDateString() : 'N/A';
...
function viewUserDetails(accountNumber) {
    const user = allUsers.find(u => u.account_number === accountNumber);
    if (!user) {
    ...
    // Get user's bills
    const userBills = allBills.filter(b => b.account_number === accountNumber);
    ...
    alert(`User Details:\n\nAccount: ${accountNumber}\nName: ${user.name}\nEmail: ${user.email || 'N/A'}\nAddress: ${user.address || 'N/A'}\nProperty Type: ${user.property_type || 'N/A'}\n\nBills:${billDetails}`);
}
...
function exportBills() {
    ...
    allBills.forEach(bill => {
        ...
        csv += `${bill.account_number},${userName},${bill.service},${parseFloat(bill.amount || 0).toFixed(2)},${dueDate},${statusText}\n`;
    });
...
function exportBillsPDF() {
    ...
    allBills.forEach(bill => {
        ...
        doc.text(bill.account_number, 20, yPos);
...
function exportReportsCSV() {
    ...
    recentPayments.forEach(payment => {
        const date = payment.created_at ? new Date(payment.created_at).toLocaleDateString() : 'N/A';
        csv += `${date},${payment.account_number},${payment.service},${parseFloat(payment.amount || 0).toFixed(2)},${payment.payment_reference || 'N/A'}\n`;
    });
