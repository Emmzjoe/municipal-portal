...
function calculatePasswordStrength(password) {
    ...
    // Calculate strength score (now includes new checks)
    if (checks.length) strength += 15;
    if (checks.uppercase) strength += 15;
    if (checks.lowercase) strength += 15;
    if (checks.number) strength += 15;
    if (checks.special) strength += 20; // Special chars worth more
    if (checks.noRepeatedChars) strength += 10;
    if (checks.noCommonWords) strength += 10;
